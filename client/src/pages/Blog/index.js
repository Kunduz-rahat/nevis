    ...
    return (
      <Layout>
        <div className='flex justify-between items-center'>
          <h2 className='text-2xl flex font-semibold leading-normal md:leading-relaxed'>NEWS</h2>
            ...
            <div key={post._id} className="  ">
              <div className="bg-white border shadow-sm px-4 py-3 rounded-lg max-w-lg">
                <div className="flex items-center">
                  ...
                  <span className="font-semibold">{<Link className='gray-200'
                                                         to={`/user/${post?.user?._id}`}>{post?.user?.name}</Link>}</span>
                    ...
                    <div className="text-gray-500 text-xs flex">
                      ...
                      <svg className="inline-block ml-1 fill-current" xmlns="http://www.w3.org/2000/svg"
                           viewBox="0 0 16 16"
                           data-supported-dps="16x16" fill="currentColor" className="mercado-match" width="16"
                           height="16"
                           focusable="false">
                        <path
                ...
                <Link to={`/news/${post._id}`} className="text-2xl font-semibold leading-normal md:leading-relaxed">{post.title}</Link>
                <p className="text-gray-800 text-sm leading-normal md:leading-relaxed">{post.description}</p>
